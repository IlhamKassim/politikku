...
4. **Verification**: Verify all changes mechanically (`.venv/bin/ruff check .`, `.venv/bin/ruff format --check .`, `.venv/bin/mypy src scripts/deepseek_agent.py`, and `.venv/bin/pytest`) before claiming completion.
